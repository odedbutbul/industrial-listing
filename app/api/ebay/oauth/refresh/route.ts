...
function parseAck(xml: string, rootKey: string): { ok: boolean; error?: string; rawErrors?: unknown[]; response?: Record<string, unknown> } {
  ...
  if (!response) return { ok: false, error: 'Invalid eBay response: ' + xml.slice(0, 500) }
...
export async function POST(request: NextRequest) {
  ...
  if (!action || !productId) {
    return NextResponse.json({ error: 'Missing params: action and productId' }, { status: 400 })
  }
  ...
  if (action === 'verify') {
    ...
    catch (err) { return NextResponse.json({ error: `Cannot connect to eBay: ${String(err)}` }, { status: 502 }) }
    ...
    return NextResponse.json({ success: true, message: 'VerifyAddItem passed - product is valid for listing' })
    ...
    if (!product.ebay_item_number) {
      return NextResponse.json({ error: 'Product not listed on eBay yet' }, { status: 400 })
    }
