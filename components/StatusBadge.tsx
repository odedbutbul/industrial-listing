...
type EbayStatus = 'pending' | 'active' | 'ended' | 'sold' | 'unsold' | 'failed'
...
const ebay: Record<EbayStatus, { label: string; cls: string }> = {
  ...
  active:   { label: '✅ פעיל',     cls: 'bg-green-100 dark:bg-green-500/15 text-green-700 dark:text-green-400' },
  ended:    { label: '🔚 הסתיים',   cls: 'bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400' },
  sold:     { label: '🏷️ נמכר',   cls: 'bg-purple-100 dark:bg-purple-500/15 text-purple-700 dark:text-purple-400' },
  unsold:   { label: '⚪ לא נמכר', cls: 'bg-gray-100 dark:bg-white/10 text-gray-500 dark:text-white/40' },
...
export function EbayBadge({ status }: { status: string }) {
  const s = ebay[status as EbayStatus] ?? ebay.pending
